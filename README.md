...
- **A new "ماده" (legal Article) heading is sometimes given the wrong level by the model** — each chunk is structured independently with no real memory of earlier chunks, so a ماده several بند/تبصره levels after its last sibling can come back nested too deep instead of matching that sibling's level, even though the correct level is visible in the breadcrumb. Fixed deterministically in `DocumentBuilder` (`align_maddeh_siblings`, default `True`, see `builder.py`): a new ماده heading is forced to match its most recent still-open ماده sibling's level, overriding the model's own level for it. This only matches text starting with "ماده" + a number, so it's a no-op on documents without ماده-numbered articles; disable it with `structure_document(..., align_maddeh_siblings=False)` or the CLI's `--no-maddeh-alignment` if you don't want it.
